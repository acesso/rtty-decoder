/**
 * Scottie S2 Line Decoder
 * Decodes a single Scottie S2 scan line using RGB sequential format
 * Scottie S2 is faster than S1 (88.064ms vs 138.24ms per channel)
 *
 * Key Features:
 * - RGB sequential encoding (no YUV conversion needed)
 * - Faster transmission than S1 (~71 seconds vs ~110 seconds total)
 * - Same negative timing structure as S1
...
export interface DecodedLine {
  ...
  width: number;
  height: number; // Always 1 for Scottie S2 (sequential RGB)
  isOddLine: boolean; // Not used for Scottie S2, always false
}

export class ScottieS2LineDecoder {
  ...
  private readonly channelSamples: number; // 88.064ms per channel (faster than S1)
  ...
  constructor(sampleRate: number) {
    // Scottie S2 timing (in seconds) - faster than S1
    const syncPulseSeconds = 0.009; // 9ms sync pulse
    const separatorSeconds = 0.0015; // 1.5ms separator
    const channelSeconds = 0.088064; // 88.064ms per color channel (R, G, B) - FASTER than S1's 138.24ms

    // First line is special: sync(9) + sep(1.5) + G(88.064) + sep(1.5) + B(88.064) + sync(9) + sep(1.5) + R(88.064)
    const firstSyncPulseSeconds = syncPulseSeconds + 2 * (separatorSeconds + channelSeconds);

    // Regular scan line: sync(9) + R(88.064) + sep(1.5) + G(88.064) + sep(1.5) + B(88.064) = 277.692ms
    ...
    // Scottie S2 transmission order: Green → Blue → [SYNC] → Red (same as S1)
    ...
    const blueBeginSeconds = blueEndSeconds - channelSeconds;     // Blue: -97.064ms to -9ms
    const greenEndSeconds = blueBeginSeconds - separatorSeconds;  // 1.5ms separator
    const greenBeginSeconds = greenEndSeconds - channelSeconds;   // Green: -186.628ms to -98.564ms
    const redBeginSeconds = separatorSeconds;                      // Red starts 1.5ms after sync
    const redEndSeconds = redBeginSeconds + channelSeconds;       // Red: +1.5ms to +89.564ms
  ...
  /**
   * Decode a single Scottie S2 scan line
   ...
   * @returns Decoded line data (1 row for Scottie S2, RGB sequential)
   */
  decodeScanLine(
    ...
    return {
      ...
      height: 1, // Scottie S2 returns ONE row per scan line (RGB sequential)
      isOddLine: false,
    };
  }

  /**
   * Get the special first sync pulse index for Scottie S2
